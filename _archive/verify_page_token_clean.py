...
def verify_page_and_token():
    ...
    try:
        ...
        if page_response.status_code == 200:
            ...
            print(f"SUCCESS: Accessed page: {page_data.get('name', 'Unknown Page')}")
            print(f"  Page ID: {page_data.get('id', 'Unknown')}")
            print(f"  Category: {page_data.get('category', 'Unknown')}")
        else:
            print(f"FAILED: Could not access page with provided ID and token: {page_response.status_code}")
            try:
        ...
        if accounts_response.status_code == 200:
            ...
            if 'data' in accounts_data:
                ...
                if not found_page:
                    print(f"\nWARNING: The page ID in your .env ({page_id}) was NOT found in the list of pages accessible with your token.")
